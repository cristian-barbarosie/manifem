...
// a cube repeated in three directions of the space
...
{	Manifold RR3 ( tag::Euclid, tag::of_dim, 3 );
	...
	// and divide RR3 by the group of translations generated by { gx, gy, gz }
	...
	Cell B = A, C = A, D = A, E = A, F = A, G = A, H = A;
	...
	Mesh AE ( tag::segment, A .reverse(), E, tag::divided_in, 15, tag::winding, gz );
	Mesh BF = AE, CG = AE, DH = AE;
	Mesh EF = AB, FG = BC, GH = CD, HE = DA;
	...
	Mesh EEE ( tag::segment, E .reverse(), EE, tag::divided_in, 8, tag::winding, -gz );
	Mesh FFF ( tag::segment, F .reverse(), FF, tag::divided_in, 8, tag::winding, -gy-gz );
	Mesh GGG ( tag::segment, G .reverse(), GG, tag::divided_in, 8, tag::winding, -gx-gy-gz );
	Mesh HHH ( tag::segment, H .reverse(), HH, tag::divided_in, 8, tag::winding, -gx-gz );
	...
	// Mesh torus_unfolded = torus .unfold ( { 0, gx, gy, gz, gx+gy, gx+gz, gy+gz, gx+gy+gz },
	// 																			tag::over_region, x*x + 2*y*y + 3*z*z < 10 );
...
{	Manifold RR3 ( tag::Euclid, tag::of_dim, 3 );
	...
	Mesh AABB ( tag::segment, AA .reverse(), BB, tag::divided_in, 2 );
	Mesh BBCC ( tag::segment, BB .reverse(), CC, tag::divided_in, 2 );
	Mesh CCDD ( tag::segment, CC .reverse(), DD, tag::divided_in, 2 );
	Mesh DDAA ( tag::segment, DD .reverse(), AA, tag::divided_in, 2 );
	Mesh AAEE ( tag::segment, AA .reverse(), EE, tag::divided_in, 2 );
	Mesh BBFF ( tag::segment, BB .reverse(), FF, tag::divided_in, 2 );
	Mesh CCGG ( tag::segment, CC .reverse(), GG, tag::divided_in, 2 );
	Mesh DDHH ( tag::segment, DD .reverse(), HH, tag::divided_in, 2 );
	Mesh EEFF ( tag::segment, EE .reverse(), FF, tag::divided_in, 2 );
	Mesh FFGG ( tag::segment, FF .reverse(), GG, tag::divided_in, 2 );
	Mesh GGHH ( tag::segment, GG .reverse(), HH, tag::divided_in, 2 );
	Mesh HHEE ( tag::segment, HH .reverse(), EE, tag::divided_in, 2 );

	Mesh ABCD ( tag::quadrangle, AABB, BBCC, CCDD, DDAA );
	Mesh EFGH ( tag::quadrangle, EEFF, FFGG, GGHH, HHEE );
	Mesh ADHE ( tag::quadrangle, DDAA .reverse(), DDHH, HHEE, AAEE .reverse() );
	Mesh BAEF ( tag::quadrangle, AABB .reverse(), AAEE, EEFF, BBFF .reverse() );
	Mesh CBFG ( tag::quadrangle, BBCC .reverse(), BBFF, FFGG, CCGG .reverse() );
	Mesh DCGH ( tag::quadrangle, CCDD .reverse(), CCGG, GGHH, DDHH .reverse() );

	Mesh cube ( tag::hexahedron, ABCD, EFGH .reverse(), ADHE, CBFG, BAEF, DCGH );
	
	Mesh torus_unfolded = cube .unfold ( tag::over_region, x*x + 2*y*y + 3*z*z < 20 );

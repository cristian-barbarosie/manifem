...
{	// begin with the usual two-dimensional space
	...
	Function::Action g_horiz ( tag::transforms, xy, tag::into, (x+1.) && y ),
	                 g_obl ( tag::transforms, xy, tag::into, (x+0.5) && (y+1.) );

	// and divide RR2 by the group of translations generated by { g_horiz, g_obl }
	Manifold torus_manif = RR2.quotient ( g_horiz, g_obl );
	...
	Mesh AB ( tag::segment, A.reverse(), B, tag::divided_in, 4 ),
	     ...
	     CA2 ( tag::segment, C.reverse(), A, tag::divided_in, 4, tag::spin, g_obl ),
	     DB2 ( tag::segment, D.reverse(), B, tag::divided_in, 4, tag::spin, g_obl - g_horiz );

	Mesh sq1 ( tag::rectangle, AB, BC, CD, DA ),
	     sq2 ( tag::rectangle, BA1, DA.reverse(), CD1.reverse(), BC.reverse(), tag::spin ),
	     sq3 ( tag::rectangle, DB2.reverse(), CD.reverse(), CA2, BA1.reverse(), tag::spin ),
	     sq4 ( tag::rectangle, CD1, DB2, AB.reverse(), CA2.reverse(), tag::spin );

...
// square periodicity, square elements, no rotation imposed, Gauss-Seidel (SOR)
...
{	Manifold RR2 ( tag::Euclid, tag::of_dim, 2 );
	...
	fe.set_integrator ( tag::Gauss, tag::quad_4 );
	...
	vector_b .setZero();  vector_sol .setZero();
	...
	for ( size_t i = 0; i < 18; i ++ )
		std::cout << "matrix[18," << i << "] = " << matrix_A .coeffRef ( 18, i ) << std::endl;
	for ( size_t i = 0; i < 18; i ++ )
		std::cout << "matrix[19," << i << "] = " << matrix_A .coeffRef ( 19, i ) << std::endl;
	for ( size_t i = 0; i < 18; i ++ )
		std::cout << "matrix[20," << i << "] = " << matrix_A .coeffRef ( 20, i ) << std::endl;

	std::cout << "now solving the system of linear equations" << std::endl;

	// Gauss-Seidel with over-relaxation - SOR
	double omega = 1.2;
	for ( size_t iter = 0; iter < 70; iter ++ )
	{  for ( size_t i = 0; i < 2*number_dofs; i ++ )
		{	double sum = 0.;
			for ( size_t j = 0; j < 2*number_dofs; j ++ )
				sum += matrix_A .coeffRef ( i, j ) * vector_sol ( j );
			double dif = ( vector_b ( i ) - sum ) / matrix_A .coeffRef ( i, i );
			vector_sol ( i ) += omega * dif;                                    }
		double sum_x = 0., sum_y = 0.;
		CellIterator it = torus.iterator ( tag::over_vertices );
		for ( it .reset(); it .in_range(); it++ )
		{	Cell V = *it;
			sum_x += vector_sol ( 2*numbering[V] );
			sum_y += vector_sol ( 2*numbering[V] + 1 );  }
		double delta_x = (  1. - sum_x ) / torus .number_of ( tag::vertices );
		double delta_y = ( -1. - sum_y ) / torus .number_of ( tag::vertices );
		for ( it .reset(); it .in_range(); it++ )
		{	Cell V = *it;
			vector_sol ( 2*numbering[V] ) += delta_x;
			vector_sol ( 2*numbering[V] + 1 ) += delta_y;  }                      }
	...
	{	Cell small_sq = *it;
		fe.dock_on ( small_sq, tag::winding );
		// run twice over the four vertices of 'small_sq'
		CellIterator it_V = small_sq.boundary().iterator ( tag::over_vertices );
		double jump_V_1 = 0., jump_V_2 = 0.; 
		for ( it_V .reset(); it_V .in_range(); it_V++ )
		{	Cell V = *it_V;

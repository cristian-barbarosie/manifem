...
{	Manifold RR2 ( tag::Euclid, tag::of_dim, 2 );
	...
	Function::Action g ( tag::transforms, theta_z, tag::into, (theta+2*pi) && z );
	Manifold cylinder_manif = RR2.quotient ( g );
	
	Cell A ( tag::vertex );  theta(A) = 0.;  z(A) = -1.;
	Cell B ( tag::vertex );  theta(B) = 0.,  z(B) =  1.;

	Mesh AA ( tag::segment, A.reverse(), A, tag::divided_in, 30, tag::spin, g );
	Mesh AB ( tag::segment, A.reverse(), B, tag::divided_in, 15 );  // no jump
	Mesh BB ( tag::segment, B.reverse(), B, tag::divided_in, 30, tag::spin, -g );
	...
	// specific information about spins is included in the three segments AA, AB and BB

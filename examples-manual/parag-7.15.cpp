...
{	Manifold RR2 ( tag::Euclid, tag::of_dim, 2 );
	...
	flip_long_segments ( torus, 1.5*d );
	// remove segments shorter than the given threshold
	
	remove_short_segments ( torus, 0.5*d );
	// remove segments shorter than the given threshold
	
	baricenters ( torus );
	// re-define the position of each vertex as baricenter of its neighbours
	
	flip_long_segments ( torus, 1.4*d );
	remove_short_segments ( torus, 0.6*d );
	baricenters ( torus );
	
	flip_long_segments ( torus, 1.35*d );
	remove_short_segments ( torus, 0.65*d );
	baricenters ( torus );

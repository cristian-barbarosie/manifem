...
{	public:
  ...
  void get_close_neighbours_of
	...
	void print_everything ( size_t indent, MetricTree * cloud );
...
{	double dist = cloud->get_dist ( this->rank );
	...
	double sum = range + dd;
	for ( int i = cloud->root->rank; i > this->rank; i-- ) std::cout << "  ";
	std::cout << "analysing (" << this->point[0] << "," << this->point[1] << ")  ";
	file_ps << "0.5 setgray ";
	file_ps << this->point[0] << " " << this->point[1] << " moveto ";
	file_ps << this->point[0] << " " << this->point[1] << " 0.3 0 360 arc fill" << std::endl;
	if ( sq_d > sum * sum ) // return;  // P is too far from 'this'
	{	std::cout << "... too far" << std::endl;  return;  }
	if ( sq_d <= dd * dd ) ll.push_back ( this->point );
	if ( sq_d <= dd * dd )
	{	std::cout << "(adding it to the list)";
		file_ps << "1 0.5 0.5 setrgbcolor ";
		file_ps << this->point[0] << " " << this->point[1] << " moveto ";
		file_ps << this->point[0] << " " << this->point[1] << " 0.3 0 360 arc fill" << std::endl;  }
	std::cout << std::endl;
	for ( typename std::list<Node*>::const_iterator it = this->children.begin();
...
template < typename Point, typename SqDist >
void MetricTree<Point,SqDist>::print_everything ( )
{	std::cout << "rank zero distance " << std::sqrt(this->sq_dist_rank_zero);
	std::cout << ", ratio " << this->ratio << std::endl;
	if ( this->root == nullptr )
	{	std::cout << "empty cloud" << std::endl;  return;  }
	std::cout << "root: ";
	this->root->print_everything ( 0, this );                     }

//-----------------------------------------------------------------------------------------------//

template < typename Point, typename SqDist >
void MetricTree<Point,SqDist>::Node::print_everything
( size_t indent, MetricTree<Point,SqDist> * cloud )

{	for ( size_t i = 0; i < indent; i++ ) std::cout << "  ";
	std::cout << "rank " << this->rank;
	std::cout << ", point (";
	std::cout << this->point[0] << "," << this->point[1] << ")";
	std::cout << ", distance " << std::sqrt(cloud->get_sq_dist(this->rank));
	std::cout << ", range " << std::sqrt(cloud->get_sq_dist(this->rank)) / cloud->range_factor << std::endl;
	for ( size_t i = 0; i < indent; i++ ) std::cout << "  ";
	...
	{	if ( this->children.size() == 1 )
			...
			std::cout << this->children.size() << " children" << std::endl;
		for ( typename std::list<Node*>::const_iterator it = this->children.begin();
		      it != this->children.end(); it++                                       )
			(*it)->print_everything ( indent+1, cloud );                                           }
}
